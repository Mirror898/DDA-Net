...
class GroupedCA(nn.Module):
    def __init__(self, channels, groups=16):
        """
        Grouped Coordinate Attention (G-CA)
        基于 EMA 代码修改：仅保留 1x1 分支 (坐标注意力)，并保持分组卷积以节省参数。
        这是消融实验中 "CA" 变体的合理实现。
        """
        super(GroupedCA, self).__init__()
        self.groups = groups
...
class CGNet(nn.Module):
    def __init__(self, in_channels=1, channel_num=16):
        ...
        self.conv_5 = DualConvBlock(channel_num * 16 * 3, channel_num * 16)

        with torch.no_grad():
            dummy_input = torch.randn(1, in_channels, 160, 192, 160)
            _, x_mov_2, x_mov_3, x_mov_4, x_mov_5 = self.encoder(dummy_input)
            embed_dim_5 = x_mov_5.shape[1]
        ...
        self.conv_fine_4 = ResConvBlock(3 + channel_num * 8 + channel_num * 8, channel_num * 8)
    ...
    def forward(self, moving, fixed):
        ...
        # Step 1: Level 5
        x_mov_5_ddsa = self.ddsa_5(x_mov_5)
        x_fix_5_ddsa = self.ddsa_5(x_fix_5)
        corr_5 = self.corr_5(x_mov_5_ddsa, x_fix_5_ddsa)
        ...
        x_mov_4_warped = self.SpatialTransformer(x_mov_4, flow_5_up)
        conv_corr_5_up = self.upsample_4(conv_corr_5)

        x_mov_4_warped_ddsa = self.ddsa_4(x_mov_4_warped)
        x_fix_4_ddsa = self.ddsa_4(x_fix_4)

        corr_4 = self.corr_4(x_mov_4_warped_ddsa, x_fix_4_ddsa)
        cat = torch.cat([x_mov_4_warped, corr_4, x_fix_4], dim=1)

        conv_corr_4 = self.conv_4(cat)
        flow_4_coarse = self.reghead_4(conv_corr_4)
        flow_4 = flow_4_coarse + flow_5_up
        ...
        cat = torch.cat([x_mov_1_warped, x_fix_1], dim=1)
