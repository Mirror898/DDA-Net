...
class DDSA(nn.Module):
    def __init__(self, channels, groups=16):
        """
        Efficient Multi-Scale Attention Module (EMA) 的3D实现
        参考 (arxiv2023)ema.py (GitHub: YOLOonMe/EMA-attention-module) 的逻辑
        """
    ...
    def forward(self, x):
        ...
        x_h_out = x_h_out.permute(0, 1, 3, 2, 4)
        x_w_out = x_w_out.permute(0, 1, 3, 4, 2)
        ...
        x12 = x2.view(b * self.groups, self.group_channels, -1)

        x21 = self.softmax(self.agp(x2).view(b * self.groups, -1, 1).permute(0, 2, 1))
        x22 = x1.view(b * self.groups, self.group_channels, -1)

        weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22))
        weights = weights.view(b * self.groups, 1, d, h, w)

        out = (group_x * weights.sigmoid()).view(b, c, d, h, w)
...
class CGNet(nn.Module):
    ...
    def forward(self, moving, fixed):
        ...
        corr_3 = self.corr_3(x_mov_3_warped, x_fix_3)
        ...
        corr_2 = self.corr_2(x_mov_2_warped, x_fix_2)
        ...
        corr_1 = self.corr_1(x_mov_1_warped, x_fix_1)
